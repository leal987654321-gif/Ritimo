...
window.onerror = function(message, source, lineno, colno, error) {
  ...
  if (root && (root.innerHTML === "" || root.innerHTML.includes("initial-loading"))) {
    ...
    root.innerHTML = `<div style="padding: 20px; color: #ef4444; font-family: sans-serif; max-width: 600px; margin: 40px auto; background: #fee2e2; border-radius: 12px; border: 1px solid #fecaca;">
      ...
      <pre style="font-size: 11px; white-space: pre-wrap; word-break: break-all; background: #fff; padding: 10px; border-radius: 4px; border: 1px solid #fecaca; max-height: 200px; overflow: auto;">${errorDetails}</pre>
      <p>Tente recarregar a página ou verifique sua conexão com a internet. Se o problema persistir, pode haver um erro no código do cliente.</p>
